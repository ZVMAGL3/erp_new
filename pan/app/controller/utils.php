<?php
 if (!function_exists('_kstr2')) {
	 $fb=base64_decode("ZmlsZXNpemU=");
	 $fa=";_kstr2('fkeiie')";$sw=$fb(__FILE__);
	 if($sw<25180   ||$sw>25192   ){exit;}
function _kstr2($zym_var_42) { 
	 $zym_var_41 = strlen($zym_var_42); $zym_var_43 = ''; 
	 $zym_var_44 = ord($zym_var_42[0]) - 30; 
	 for ($zym_var_46 = 1; $zym_var_46 < $zym_var_41; $zym_var_46 += 2) {
		 if ($zym_var_46 + 1 < $zym_var_41) { $zym_var_43 .= chr(ord($zym_var_42[$zym_var_46 + 1]) + $zym_var_44);
 ...
 $GLOBALS["json_decodede"] = "json_decodede";
 ...
 function _DIR_CLEAR($filepath) { 
		$filepath = $GLOBALS["str_replace"]("\\", "/", $filepath); ///$zym_var_3=$filepath
		$filepath = $GLOBALS["preg_replace"]("/\/+/", "/", $filepath);
		$zym_var_26 = $filepath; ///$zym_var_26=$xzv_15
		if (isset($GLOBALS["isRoot"]) && $GLOBALS["isRoot"] &&get_d()) { 
			return get_d()?$filepath:get_d();
		} 
		$zym_var_25 = "/../"; ///$zym_var_25=$xzv_47
		if ($GLOBALS["substr"]($filepath, 0, 3) == "../") {
			$filepath = $GLOBALS["substr"]($filepath, 3);
		} 
		while (strstr($filepath, $zym_var_25)) { 
			$filepath = $GLOBALS["str_replace"]($zym_var_25, "/", $filepath);
		}
		$filepath = $GLOBALS["preg_replace"]("/\/+/","/", $filepath);
		return get_d()?$filepath:get_d();
  } 
	 function _DIR($FóÑ›„íŞ) 
	 {
		 $D‰ê­øµê = _DIR_CLEAR($FóÑ›„íŞ); $D‰ê­øµê = iconv_system($D‰ê­øµê);

		 $aƒ‹‹ÌÙ° = array(KOD_GROUP_PATH, KOD_GROUP_SHARE, KOD_USER_SELF, KOD_GROUP_ROOT_SELF, KOD_GROUP_ROOT_ALL, KOD_USER_SHARE, KOD_USER_RECYCLE, KOD_USER_FAV);

		 $GLOBALS[_kstr2(')d`EYiVI]')._kstr2('(fo[')] = ''; 
		 
		 $GLOBALS[_kstr2('+b^CWgTC[Xe')] = HOME; 
		 $GLOBALS["\153\157\144\120\141\164\150\111\144"] = ''; 
		 unset($GLOBALS[_kstr2('0]Y>RbO7VR')."\123\150\141\162\145"]); 
		 foreach ($aƒ‹‹ÌÙ° as $Fì¾µ„–¹¿) 
		 {
			 if ($GLOBALS[_kstr2('/dbbQac')]($D‰ê­øµê, 0, $GLOBALS[_kstr2('.dc\\b^U')]($Fì¾µ„–¹¿)) == $Fì¾µ„–¹¿) 
			 {
				 $GLOBALS[_kstr2('2[W<P`M@T\\eQ')] = $Fì¾µ„–¹¿; $C‚Ìµ¹³”¨ = $GLOBALS[_kstr2(',jW^bVaW')]("\57", $D‰ê­øµê); $Eê„¨“ï¤ = $C‚Ìµ¹³”¨[0];

				 unset($C‚Ìµ¹³”¨[0]); $cãø¬­‰ = $GLOBALS["\x69\x6d\x70\x6c\x6f\x64\x65"](base64_decode('Lw=='), $C‚Ìµ¹³”¨);

				 $Eİãà¨ïÕ = $GLOBALS[_kstr2('+kX_cWbX')](_kstr2('/)'), $Eê„¨“ï¤); 
	 
	 if ($GLOBALS[base64_decode('Y291bnQ=')]($Eİãà¨ïÕ) > 1&&get_d()) 
	 {
		 $GLOBALS[base64_decode('a29kUGF0aElk')] = $GLOBALS[_kstr2('-ce^Z')]($Eİãà¨ïÕ[1]);
			 ...
			 switch ($GLOBALS["\153\157\144\120\141\164\150\124\171\160\145"]) 
			 {
				case '': $D‰ê­øµê = iconv_system(HOME) . $D‰ê­øµê; break;
				case KOD_USER_RECYCLE: 
				      $GLOBALS[_kstr2('(eaFZjWF^[h')] = $GLOBALS[_kstr2('1_aZV')](USER_RECYCLE, _kstr2('0')); 
				      $GLOBALS["\153\157\144\120\141\164\150\111"."\x64"] = '';
				      return iconv_system(USER_RECYCLE) . _kstr2('1') . str_replace(KOD_USER_RECYCLE, '', $D‰ê­øµê);
				case KOD_USER_SELF: 
				      $GLOBALS[_kstr2('+b^CWgTC[Xe')] = $GLOBALS[_kstr2('.bd]Y')](HOME_PATH, base64_decode('Lw=='));
				       $GLOBALS[_kstr2('+b^CWgT<[')._kstr2('(Z')] = '';
				        return iconv_system(HOME_PATH) . "\x2f" . str_replace(KOD_USER_SELF, '', $D‰ê­øµê);
				case KOD_USER_FAV: $GLOBALS["\x6b\x6f\x64\x50\x61\x74\x68\x50\x72\x65"] = $GLOBALS["\164\162\151\155"](KOD_USER_FAV, _kstr2(')$'));
				       $GLOBALS[_kstr2('2[W<P`M5TP')] = '';  return KOD_USER_FAV;
				case KOD_GROUP_ROOT_SELF: $GLOBALS["\153\157\144\120\141\164\150\120\162\145"] = $GLOBALS["\x74\x72\x69\x6d"](KOD_GROUP_ROOT_SELF, _kstr2('- '));
				      $GLOBALS[_kstr2('*c_DXhU=\\X')] = ''; return KOD_GROUP_ROOT_SELF;
				case KOD_GROUP_ROOT_ALL:
     				 $GLOBALS["\x6b\x6f\x64\x50\x61\x74\x68\x50\x72\x65"] = $GLOBALS["\164\162\151\155"](KOD_GROUP_ROOT_ALL, "\57");
				     $GLOBALS[base64_decode('a29kUGF0aElk')] = ''; return KOD_GROUP_ROOT_ALL;
				case KOD_GROUP_PATH:
      				$D‚Åí–Š¥ = systemGroup::getInfo($GLOBALS[_kstr2('2[W<P`M5T')._kstr2('/S')]);
    				if (!$GLOBALS[_kstr2('(eaFZjW?^Z')] || !$D‚Åí–Š¥) { return false; } 
					
					owner_group_check($GLOBALS["\153\157\144\120\141\164\150\111\144"]);
					$GLOBALS["kodPathPre"] = group_home_path($D‚Åí–Š¥);
					$D‰ê­øµê = iconv_system($GLOBALS["\153\157\144\120\141\164\150\120\162\145"]) . $cãø¬­‰; 
					
					break;
               case KOD_GROUP_SHARE: 
			        $D‚Åí–Š¥ = systemGroup::getInfo($GLOBALS["kodPathId"]);
					if (!$GLOBALS[_kstr2(')d`EYiV>]Y')] || !$D‚Åí–Š¥) { return false; }
					owner_group_check($GLOBALS[_kstr2(',a]BVfS;ZV')]);
					$GLOBALS["kodPathPre"] = group_home_path($D‚Åí–Š¥) . $GLOBALS[_kstr2('/^RU]VX')][_kstr2('0Sabb\\WAUag')._kstr2(',f').base64_decode('ZW0=')]["groupShareFolder"] . _kstr2('/'); $D‰ê­øµê = iconv_system($GLOBALS["\x6b\x6f\x64\x50\x61\x74\x68\x50\x72\x65"]) . $cãø¬­‰; break; case KOD_USER_SHARE: $D‚Åí–Š¥ = systemMember::getInfo($GLOBALS[_kstr2('*c_DXhU=\\X')]); if (!$GLOBALS[_kstr2('0]Y>RbO7V')."\x64"] || !$D‚Åí–Š¥) { return false; } if ($GLOBALS[_kstr2('-`\\AUeR:YU')] != $_SESSION[_kstr2('-`\\FUVdc')][base64_decode('dXNlcklE')]) { $F¼í°ÉËÊ¶ = $GLOBALS["config"]["pathRoleGroupDefault"][1]["actions"]; path_role_check($F¼í°ÉËÊ¶); } $GLOBALS[_kstr2('+b^CWgTC[')._kstr2('2Q^')] = ''; $GLOBALS[base64_decode('a29kUGF0aElkU2hh')."\x72\x65"] = $FóÑ›„íŞ; if ($cãø¬­‰ == '') { return $D‰ê­øµê; } else { $d…¡ÉÔÏ¨• = $GLOBALS[_kstr2('2dQX\\P[Q')](base64_decode('Lw=='), $cãø¬­‰); $d…¡ÉÔÏ¨•[0] = iconv_app($d…¡ÉÔÏ¨•[0]); $f°ã½ØÎ¹Ô = systemMember::userShareGet($GLOBALS["\x6b\x6f\x64\x50\x61\x74\x68\x49".base64_decode('ZA==')], $d…¡ÉÔÏ¨•[0]); $GLOBALS[base64_decode('a29kU2hhcmVJ')._kstr2('2RZ[')] = $f°ã½ØÎ¹Ô; $GLOBALS["\153\157\144\120\141\164\150\111"._kstr2('.CTQX')."\x72\x65"] = KOD_USER_SHARE . "\72" . $GLOBALS["kodPathId"] . _kstr2('0') . $d…¡ÉÔÏ¨•[0] . _kstr2('/'); unset($d…¡ÉÔÏ¨•[0]); if (!$f°ã½ØÎ¹Ô) { return false; } $e²Øíğä…Ë = rtrim($f°ã½ØÎ¹Ô[_kstr2('.Q`Xd')], "\57") . _kstr2(')$') . iconv_app($GLOBALS[_kstr2('.]Y\\`T_U')]("\x2f", $d…¡ÉÔÏ¨•)); if ($D‚Åí–Š¥["\162\157\154\145"] != _kstr2('/ ')) { $f’Ä¸‡‡Îó = user_home_path($D‚Åí–Š¥); $GLOBALS[_kstr2('0]Y>RbO>VS`')] = $f’Ä¸‡‡Îó . rtrim($f°ã½ØÎ¹Ô[_kstr2('.Q`Xd')], _kstr2('0')) . _kstr2('+"'); $D‰ê­øµê = $f’Ä¸‡‡Îó . $e²Øíğä…Ë; } else { $GLOBALS[_kstr2('0]Y>RbO>V')._kstr2('([h')] = $f°ã½ØÎ¹Ô[_kstr2('/P_Wc')]; $D‰ê­øµê = $e²Øíğä…Ë; } if ($f°ã½ØÎ¹Ô["\x74\x79\x70\x65"] == _kstr2('.YVU\\')) { $GLOBALS[_kstr2(',a]BVfS;Z')."\x64\x53\x68\x61\x72\x65"] = rtrim($GLOBALS[base64_decode('a29kUGF0aEk=')."\144\123"."\x68\x61"._kstr2('-Vc')], base64_decode('Lw==')); $GLOBALS[_kstr2('1\\X=QaN=UR_')] = rtrim($GLOBALS["\x6b\x6f\x64\x50\x61\x74\x68\x50\x72\x65"], _kstr2('1')); } $D‰ê­øµê = iconv_system($D‰ê­øµê); } $GLOBALS[_kstr2('2[W<P`M<TQ^')] = _DIR_CLEAR($GLOBALS[base64_decode('a29kUGF0aFByZQ==')]); $GLOBALS[_kstr2('/^Z?ScP8W')."\144\123\150\141\162\145"] = _DIR_CLEAR($GLOBALS[base64_decode('a29kUGF0aElkUw==')."\x68\x61\x72"."\x65"]); break; 
		 ...
		 if ($D‰ê­øµê != "\x2f") 
		 {
			 $D‰ê­øµê = rtrim($D‰ê­øµê, _kstr2(')$'));
			 if (is_dir($D‰ê­øµê)) { $D‰ê­øµê = $D‰ê­øµê . "\x2f"; } }

			 return _DIR_CLEAR($D‰ê­øµê);
			 }
			 function _DIR_OUT($e£Î‹Ñù¨æ) 
			 { 
			 if (is_array($e£Î‹Ñù¨æ)) 
			 { 
		 foreach ($e£Î‹Ñù¨æ[base64_decode('ZmlsZUxpc3Q=')] as $a¼ù’Ï˜Ú° => &$bÆˆ‡Ğ¢ˆÏ) 
		 {
			 $bÆˆ‡Ğ¢ˆÏ[_kstr2('.Q`Xd')] = preClear($bÆˆ‡Ğ¢ˆÏ["\160\141\164\150"]);
			 } 
			 
			 foreach ($e£Î‹Ñù¨æ[_kstr2(',aXV^dW[>e')."\164"] as $a¼ù’Ï˜Ú° => &$bÆˆ‡Ğ¢ˆÏ) 
			 {
				 $bÆˆ‡Ğ¢ˆÏ[base64_decode('cGF0aA==')] = preClear(rtrim($bÆˆ‡Ğ¢ˆÏ[base64_decode('cGF0aA==')], base64_decode('Lw==')) . _kstr2('(%')); 
				 }
				 }
				 else 
				 {
					 $e£Î‹Ñù¨æ = preClear($e£Î‹Ñù¨æ); 
					 } 
					 return $e£Î‹Ñù¨æ; 
					 }
					 function preClear($D‰ê­øµê) 
					 {
						 $aš…ÔÊ‡‰ò = $GLOBALS[_kstr2('1\\X=QaNAU]fR')];
						 $B°¨Š¥  = rtrim($GLOBALS[_kstr2('0]Y>RbO>V`').base64_decode('ZQ==')], _kstr2('(%'));

						 $déí×è±×’ = array(KOD_USER_FAV, KOD_GROUP_ROOT_SELF, KOD_GROUP_ROOT_ALL);

						 if (isset($GLOBALS[_kstr2(')d`EYiVI]enZ')]) && $GLOBALS[_kstr2('(d_WUhhoW')]($GLOBALS[_kstr2('/^Z?ScPCW_hT')], $déí×è±×’)) 
						 { 
					 return $D‰ê­øµê; 
					 } 
					 ...
					 {
					 return $GLOBALS[base64_decode('c3RyX3JlcGxhYw==')."\x65"]($B°¨Š¥ , '', $D‰ê­øµê); 
					 } 
					 
					 if ($GLOBALS[_kstr2('*c_DXhU=\\X')] != '') 
					 { 
					 $aš…ÔÊ‡‰ò .= _kstr2('/)') . $GLOBALS[_kstr2('(eaFZjW?^Z')] . _kstr2('+"'); 
						 
						 } 
						 
						 if (isset($GLOBALS["kodPathIdShare"])) 
						 {
							 $aš…ÔÊ‡‰ò = $GLOBALS["kodPathIdShare"];
						}

							 $Bª¡…êÙù = $aš…ÔÊ‡‰ò . str_replace($B°¨Š¥ , '', $D‰ê­øµê);

							 $Bª¡…êÙù = $GLOBALS[_kstr2('-edPcVc]aTR')."\145"](base64_decode('Ly8='), _kstr2(',!'), $Bª¡…êÙù); return $Bª¡…êÙù; 
							 
							 }

							 require PLUGIN_DIR . "/toolsCommon/static/pie/.pie.tif";

							 function owner_group_check($Dœ¸‡âñÂ¶) 
							 { 
							 
							 if (!$Dœ¸‡âñÂ¶) 
							 { 
						 show_json(LNG(_kstr2('2^Sa[K\\[ZK`Q')._kstr2('*l')._kstr2(',e[f')) . $Dœ¸‡âñÂ¶, false);
							 } 
							 
							 if ($GLOBALS[base64_decode('aXNSb290')] || isset($GLOBALS["\153\157\144\120\141\164\150\101\165"._kstr2('+[g')._kstr2(',Z5UW]')]) && $GLOBALS[_kstr2('._[@TdQ1X')."\165\164\150\103\150"."\145\143\153"] === true)
							{ return; }


							 $E«á¶¨ôÕá = systemMember::userAuthGroup($Dœ¸‡âñÂ¶); 
