function initData() {
    if (window.location.search.indexOf("?") == 0 && window.location.search.indexOf("=") > 1 & window.location.search.indexOf("watermark_txt") != -1) {
        var markUrl = new Object();
        if (window.location.search.indexOf("watermark_txt") != -1) {
            markUrl.watermark_txt = getQueryString("watermark_txt");
        }
        if (window.location.search.indexOf("watermark_x_extra") != -1) {
            markUrl.watermark_x_extra = getQueryString("watermark_x_extra");
        }
        if (window.location.search.indexOf("watermark_y_extra") != -1) {
            markUrl.watermark_y_extra = getQueryString("watermark_y_extra");
        }
        if (window.location.search.indexOf("watermark_fontsize") != -1) {
            markUrl.watermark_fontsize = getQueryString("watermark_fontsize");
        }
        if (window.location.search.indexOf("watermark_alpha") != -1) {
            markUrl.watermark_alpha = getQueryString("watermark_alpha");
        }
        if (window.location.search.indexOf("watermark_angle") != -1) {
            markUrl.watermark_angle = getQueryString("watermark_angle");
        }
        if (window.location.search.indexOf("watermark_font") != -1) {
            markUrl.watermark_font = getQueryString("watermark_font");
        }
        if (window.location.search.indexOf("watermark_color") != -1) {
            markUrl.watermark_color = getQueryString("watermark_color");
        }
        watermark(markUrl);
    } else {
...
function maxtHandle(element) {
    var maxData;
    var maxDataArr = [];
    $.each(element, function (index, ele) {
        var thisWidthtArr = ele.clientWidth;
        var thisHeightArr = ele.clientHeight;
        maxDataArr.push(thisWidthtArr);
        maxDataArr.push(thisHeightArr);
    })
    maxData = maxDataArr.sort(function (a, b) {
        return a - b;
    });
    return maxData[maxDataArr.length - 1];
}
...
function watermark(settings) {
    ...
    var defaultSettings = {
        ...
        watermark_y_space: 0,//水印y轴最小间隔
        watermark_x_extra: 100,//水印x轴实际间隔
        watermark_y_extra: 105,//水印y轴实际间隔
    ...
    var elementArr = $('.word-page');
    //获取页面最大宽度
    var page_width = maxtHandle(elementArr);
    //获取页面最大长度
    var page_height = maxtHandle(elementArr);
