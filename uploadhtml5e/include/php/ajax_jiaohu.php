...
include("../../../inc/B_database.php");
error_reporting(E_ALL & ~ E_NOTICE);	//屏蔽没有必要的错误提示，如变量未定义
date_default_timezone_set('PRC');//设成北京时间
define("DB_QZ",$database["tableqz"]);//表前缀
define("DB_IP",$database["host"]); //数据库ip
define("DB_ROOT",$database["username"]);      //数据库登录名
define("DB_PASSWORD",$database["password"]);  //数据库登录密码
define("DB_NAME", $database["database"]); //数据库名
...
if($act=="del"){
   ...
   $idno=$mydb->checksql(trim($_POST["idno"]));
	if($idno!=""){
		$info=$mydb->selectall("select * from sys_file where idno='".$idno."'");
		if($info){
			$path=$info[0]["src"];
			$mydb->query("delete from sys_file  where id=".$info[0]["id"]);
		}
	}else{
		$path=trim($_POST["src"]); 
	}
...
if($act=="up"){
	...
	if($res["status"]=="success"){
		...
		$images_id=$mydb->add("sys_file",$img_data);//录入一条记录
...
if($act=="sort"){
   ...
   for($i=0;$i<$num;$i++){
		...
		$mydb->update("`sys_file`",$data," where idno='".$idno."' "); 
		$successnum++;
   }
...
if($act=="list"){
	...
	$data=$mydb->selectall("select size,ext,filetype,id,idno,tidno,src,title as name from sys_file where tidno='".$tidno."' order by sort asc,id desc");
	$res["data"]=$data;
	echo json_encode($res);
	exit();
