...
function update_sort(childs){
	...
	var index=0;
		  ...
		  if(index>0){
				 data["num"]=index;
				 $.ajax({
				 url:conf_jiaohu.uploadurl+"?act=sort",
				 type:"post",
				 data:data,
				 async:false,
				 dataType:"json",
...
function createupload_jiaohu(opt){
	// console.log(opt)
	var tidno=$("#"+opt.inputid+"_"+opt.ToHtmlID).attr("tidno");   //得到文本框tidno
	var NowToHtmlID=$("#"+opt.ToHtmlID);
	var strmk_id = parseInt($('#'+opt.ToHtmlID+"_content_foot").find('#post_form').attr('strmk_id'));      //得到修改的id值
	if (isNaN(strmk_id)) {
		strmk_id = 0;
	};
	
	var baseinfo={tidno:tidno,type:"0",classid:"12"}; //传参数给ajax_sql.asp  tidno 为关联表的唯一序号 classid为分类id
	var myhcfile=new hcfile();
	var mytimer=null;
	// console.log(conf_jiaohu.uploadurl+"?act=up")
	myhcfile.Init({
		url:conf_jiaohu.uploadurl+"?act=up",
		maxnum:50,//只能上传N张
		type:"all",//yinpin:仅上传音频文件,shipin:仅上传视频文件,img:仅上传图片,不填或all表示全部
		input_id:opt.inputid+"_"+opt.ToHtmlID,//存多张图片路径的input的id
		input_fengmian_id:'',//存封面的图片路径input对象id
		weizhi_id:opt.inputid+"_"+opt.ToHtmlID+"_weizhi_1",//选择按钮位置对像id
		panel_id:opt.inputid+"_"+opt.ToHtmlID+"_panel_1",//图片文件显示对象id
		...
		item_class:"myitema", //项样式类名 hcfile03.css里有内置的类名
		...
		down_url:conf.downurl,//可以不填写，下载的
		bt_del_text:"X",
		item_yifu_id:opt.inputid+"_"+opt.ToHtmlID+"_weizhi_1",//项依附id,依附的上级元素必须跟项的上级元素同一个元素
		...
		upload_show:{date:0,size:0,name:0,bar:0,baifenbi:0,del:1,bg:0},//上传过程中要显示的元素,1:显示,0不显示 date(日期) size(大小) name(标题) bar(进度条) baifenbi(进度条) del(删除按钮)
		...
		delete_huidiao:function(op){//成功上传回调
			...
			if(confirm("删除？")){
				// console.log(myitem.getAttribute("data-idno"))
				$.post(conf_jiaohu.uploadurl + "?act=del",{
					idno: myitem.getAttribute("data-idno")
				},
				function (res) {
					res = JSON.parse(res)
					// console.log(res);
		...
		fengmian_huidiao:function(op){//点击封面回调事件
				...
				var myitem=op.myitem;
				$.ajax({
					url:conf_jiaohu.uploadurl+"?act=setfengmian",
					type:"get",
	...
	//加载初始化显示图片
	myhcfile.ajax({
		url:conf_jiaohu.uploadurl+"?act=list",
		type:"get",
		data:baseinfo,
		async:false,
		dataType:"json",
		success:function(res){
			...
			for(var i=0;i<data.length;i++){
				var row=data[i];
				myhcfile.NewItem({type:"first",row:row,inserttype:"append"});
			};
			// myhcfile.ResetInput();
		}
	});
	...
	mydd.init({
		listid:opt.inputid+"_"+opt.ToHtmlID+"_panel_1" //项所在的父类元素id
