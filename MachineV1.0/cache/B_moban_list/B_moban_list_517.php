...
global $const_q_cak,$ToHtmlID,$const_q_shanc,$const_q_xiug,$nowkeyword,$px_ziduan,$pxv,$Connadmin,$hy;
	...
	$const_shaixuan="";
	$SYS_ALL_ziduan_list="id,sys_title,sys_hangye,sys_name,sys_file,sys_beizhu,sys_Id_MenuBigClass,sys_nowbh,sys_id_login,sys_shenpi_all";
	$xianshi_ZD_num="7";
	$xianshi_KD_num="1350";
	$shuoding_num="1";
	$maxrecord="50";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="id,sys_title,sys_hangye,sys_name,sys_file,sys_beizhu,sys_Id_MenuBigClass,sys_id_zu,sys_id_fz,sys_yfzuid,sys_bh,sys_zt,sys_zt_bianhao,sys_nowbh,sys_huis,sys_id_bumen,sys_web_shenpi,sys_adddate,sys_adddate_g,sys_id_login,sys_login,sys_shenpi,sys_shenpi_all,sys_chaosong,sys_paixu";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,msc_RenZhengMoBan.id  from `msc_RenZhengMoBan`,(select @rownum:=0) as msc_RenZhengMoBan where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,sys_title,sys_hangye,sys_name,sys_file,sys_beizhu,sys_Id_MenuBigClass,sys_nowbh,sys_id_login,sys_shenpi_all  from `msc_RenZhengMoBan` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      $sql .= " order by $px_ziduan $pxv";
      $rs = mysqli_query( $Connadmin, $sql );
      echo( "<div id='para'  tabindex='-1' class='tables'  style='border:0;border-bottom:1px solid #CCC;min-width:100%;width: 1350px'>" );
	      ...
	      if ( $const_q_cak >= 0 or $now_id_login == $bh ) {
			...
			//}
			echo( "  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;min-width:100%;width:1350px'> " );
			echo( "<li class='cols1 shuodingli'  title='$now_id'  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;text-align:center;'></li>" );
              ...
              echo("<li id='c_tdtop1'  class='contentli ET_sys_title$now_id F_M_XS_1' ET='ET_sys_title'  xstypeid='1' name='sys_title' style='height:25px;line-height:25px;width:200px;'>".DeleteHtml($row['sys_title'])."</li>");
              echo("<li id='c_tdtop1'  class='contentli ET_sys_hangye$now_id F_M_XS_1' ET='ET_sys_hangye'  xstypeid='1' name='sys_hangye' style='height:25px;line-height:25px;width:120px;'>".DeleteHtml($row['sys_hangye'])."</li>");
              echo("<li id='c_tdtop1'  class='contentli ET_sys_name$now_id F_M_XS_1' ET='ET_sys_name'  xstypeid='1' name='sys_name' style='height:25px;line-height:25px;width:150px;'>".DeleteHtml($row['sys_name'])."</li>");
              echo("<li id='c_tdtop1'  class='contentli ET_sys_file$now_id F_M_XS_1' ET='ET_sys_file'  xstypeid='1' name='sys_file' style='height:25px;line-height:25px;width:150px;'>".DeleteHtml($row['sys_file'])."</li>");
              echo("<li id='c_tdtop1'  class='contentli ET_sys_beizhu$now_id F_M_XS_1' ET='ET_sys_beizhu'  xstypeid='1' name='sys_beizhu' style='height:25px;line-height:25px;width:200px;'>".DeleteHtml($row['sys_beizhu'])."</li>");
              echo("<li id='c_tdtop1'  class='contentli ET_sys_Id_MenuBigClass$now_id F_M_XS_1' ET='ET_sys_Id_MenuBigClass'  xstypeid='1' name='sys_Id_MenuBigClass' style='height:25px;line-height:25px;width:150px;'>".DeleteHtml($row['sys_Id_MenuBigClass'])."</li>");
