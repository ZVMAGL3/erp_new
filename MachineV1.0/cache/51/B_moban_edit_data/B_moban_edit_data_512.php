...
if ( $const_q_xiug >= 0 ) { //有修改权限时
    ...
    //-----------------------------------------------------------查询原记录
    $sql2 = "select * From  `SQP_ZhengCeFaGui`  where `id`='$strmk_id'";
    $rs2 = mysqli_query( $Conn, $sql2 );
    $row2 = mysqli_fetch_array( $rs2 );
	$Y_ZD_LianJie=$row2['ZD_LianJie'];
	...
	//-----------------------------------------------------------更新记录
	$sql = "UPDATE  `SQP_ZhengCeFaGui`  set `ZD_LianJie`='$ZD_LianJie',`ZD_FuJian`='$ZD_FuJian',`ZD_WenJianMingChen`='$ZD_WenJianMingChen',`sys_gx_id_198`='$sys_gx_id_198',`sys_adddate_g`='$nowdates' where `id`='$strmk_id'";
	mysqli_query( $Conn,$sql );


    //------------------------执行更新对比
    $sys_editcontent='';
	if($Y_ZD_LianJie!=$ZD_LianJie){
		$sys_editcontent.='链接:  '.$Y_ZD_LianJie.'=>'.$ZD_LianJie.';</br>';
	};
...
if($sys_editcontent!=''){
	...
	$sys_postvalue_list = "'512','$strmk_id','$sys_editcontent'";		
	Jilu_add_Modular( 'sys_xiuguaijilu', $sys_postzd_list, $sys_postvalue_list); //添加数据 并生成添加的id
	LiYi_Add(512,$strmk_id,535,'+');       //利益函数 这里奖励货币
}


    Sys_XuanYongMoban(512,$strmk_id);//利益函数 这里奖励货币 
