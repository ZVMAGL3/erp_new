...
if ( $const_q_xiug >= 0 ) { //有修改权限时
    ...
    //-----------------------------------------------------------查询原记录
    $sql2 = "select * From  `sys_Work`  where `id`='$strmk_id'";
	...
	//-----------------------------------------------------------更新记录
	$sql = "UPDATE  `sys_Work`  set ,`sys_adddate_g`='$nowdates' where `id`='$strmk_id'";
...
if($sys_editcontent!=''){
	...
	$sys_postvalue_list = "'515','$strmk_id','$sys_editcontent'";		
	Jilu_add_Modular( 'sys_xiuguaijilu', $sys_postzd_list, $sys_postvalue_list); //添加数据 并生成添加的id
	LiYi_Add(515,$strmk_id,535,'+');       //利益函数 这里奖励货币
}


    Sys_XuanYongMoban(515,$strmk_id);//利益函数 这里奖励货币 
