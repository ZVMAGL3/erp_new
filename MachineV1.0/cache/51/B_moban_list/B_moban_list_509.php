...
global $const_q_cak,$ToHtmlID,$const_q_shanc,$const_q_xiug,$nowkeyword,$px_ziduan,$pxv,$Conn,$hy;
	$r_cow_height="28";
	$databiao="SQP_KuaiDiShouFaDengJi";
	$const_shaixuan="";
	$SYS_ALL_ziduan_list="id,sys_nowbh,ZD_GongSiMingChen,ZD_NaRong,ZD_RiQi,ZD_KuaiDiMingChen,ZD_KuaiDiDanHao,ZD_GenZongQingKuang,sys_id_login,sys_shenpi_all";
	$xianshi_ZD_num="7";
	$xianshi_KD_num="1070";
	$shuoding_num="2";
	$maxrecord="50";
	$FormattingXianShi_idlist="1,2,19";
	$zu_all_list="";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="id,sys_id_login,sys_login,sys_id_zu,sys_id_fz,sys_yfzuid,sys_bh,sys_zt,sys_zt_bianhao,sys_nowbh,sys_huis,sys_id_bumen,sys_shenpi,sys_web_shenpi,sys_adddate,sys_adddate_g,sys_shenpi_all,ZD_GongSiMingChen,ZD_NaRong,ZD_RiQi,ZD_KuaiDiMingChen,ZD_KuaiDiDanHao,ZD_GenZongQingKuang,sys_chaosong,sys_paixu";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,SQP_KuaiDiShouFaDengJi.id  from `SQP_KuaiDiShouFaDengJi`,(select @rownum:=0) as SQP_KuaiDiShouFaDengJi where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,sys_nowbh,ZD_GongSiMingChen,ZD_NaRong,ZD_RiQi,ZD_KuaiDiMingChen,ZD_KuaiDiDanHao,ZD_GenZongQingKuang,sys_id_login,sys_shenpi_all  from `SQP_KuaiDiShouFaDengJi` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      ...
      echo( "<div id='para'  tabindex='-1' class='tables'  style='border:0;border-bottom:1px solid #CCC;min-width:100%;width: 1070px'>" );
	      ...
	      if ( $const_q_cak >= 0 or $now_id_login == $bh ) {
			...
			echo( "<li class='cols1 shuodingli'  title='$now_id'  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;text-align:center;'></li>" );
              ...
              echo("<li  class='contentli ET_ZD_NaRong$now_id F_M_XS_2' ET='ET_ZD_NaRong'  xstypeid='2' name='ZD_NaRong' style='height:28px;line-height:28px;width:200px;'>".DeleteHtml($row['ZD_NaRong'])."</li>");
              echo("<li  class='contentli ET_ZD_RiQi$now_id F_M_XS_1' ET='ET_ZD_RiQi'  xstypeid='1' name='ZD_RiQi' style='height:28px;line-height:28px;width:70px;'>".DeleteHtml($row['ZD_RiQi'])."</li>");
              echo("<li  class='contentli ET_ZD_KuaiDiMingChen$now_id F_M_XS_1' ET='ET_ZD_KuaiDiMingChen'  xstypeid='1' name='ZD_KuaiDiMingChen' style='height:28px;line-height:28px;width:60px;'>".DeleteHtml($row['ZD_KuaiDiMingChen'])."</li>");
              echo("<li  class='contentli ET_ZD_KuaiDiDanHao$now_id F_M_XS_1' ET='ET_ZD_KuaiDiDanHao'  xstypeid='1' name='ZD_KuaiDiDanHao' style='height:28px;line-height:28px;width:150px;'>".DeleteHtml($row['ZD_KuaiDiDanHao'])."</li>");
              echo("<li  class='contentli ET_ZD_GenZongQingKuang$now_id F_M_XS_19' ET='ET_ZD_GenZongQingKuang'  xstypeid='19' name='ZD_GenZongQingKuang' style='height:28px;line-height:28px;width:60px;'>".DeleteHtml($row['ZD_GenZongQingKuang'])."</li>");
