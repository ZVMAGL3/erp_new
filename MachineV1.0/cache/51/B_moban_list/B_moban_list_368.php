...
global $const_q_cak,$ToHtmlID,$const_q_shanc,$const_q_xiug,$nowkeyword,$px_ziduan,$pxv,$Conn,$hy;
	$r_cow_height="28";
	$databiao="sys_xiuguaijilu";
	$const_shaixuan="";
	$SYS_ALL_ziduan_list="id,sys_re_id,sys_edit_id,sys_editcontent,sys_nowbh";
	$xianshi_ZD_num="4";
	$xianshi_KD_num="898";
	$shuoding_num="1";
	$maxrecord="50";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="id,sys_re_id,sys_edit_id,sys_editcontent,sys_id_login,sys_login,sys_id_zu,sys_id_fz,sys_yfzuid,sys_bh,sys_zt,sys_zt_bianhao,sys_nowbh,sys_huis,sys_id_bumen,sys_shenpi,sys_web_shenpi,sys_adddate,sys_adddate_g,sys_shenpi_all,sys_chaosong,sys_paixu";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,sys_xiuguaijilu.id  from `sys_xiuguaijilu`,(select @rownum:=0) as sys_xiuguaijilu where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,sys_re_id,sys_edit_id,sys_editcontent,sys_nowbh  from `sys_xiuguaijilu` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      $sql .= " order by $px_ziduan $pxv";
      $rs = mysqli_query( $Conn, $sql );
      echo( "<div id='para'  tabindex='-1' class='tables'  style='border:0;border-bottom:1px solid #CCC;min-width:100%;width:898'>" );
	      ...
	      if ( $const_q_cak >= 0 ) {
			...
			echo( "<li class='cols1 shuodingli'  title='$now_id'  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;text-align:center;'></li>" );
              ...
              echo("<li class='contentli ET_sys_re_id$now_id F_M_XS_1' ET='ET_sys_re_id'  xstypeid='1' name='sys_re_id' style='height:28px;line-height:28px;width:112px;'>".DeleteHtml($row['sys_re_id'])."</li>");
              echo("<li class='contentli ET_sys_edit_id$now_id F_M_XS_1' ET='ET_sys_edit_id'  xstypeid='1' name='sys_edit_id' style='height:28px;line-height:28px;width:81px;'>".DeleteHtml($row['sys_edit_id'])."</li>");
              echo("<li class='contentli ET_sys_editcontent$now_id F_M_XS_1' ET='ET_sys_editcontent'  xstypeid='1' name='sys_editcontent' style='height:28px;line-height:28px;width:534px;'>".DeleteHtml($row['sys_editcontent'])."</li>");
