...
global $const_q_cak,$ToHtmlID,$const_q_shanc,$const_q_xiug,$nowkeyword,$px_ziduan,$pxv,$Conn,$hy;
	$r_cow_height="28";
	$databiao="SQP_NaWaiBuYinSuXiangGuanXinXiYiLanBiao";
	$const_shaixuan="";
	$SYS_ALL_ziduan_list="id,sys_nowbh,ZD_test,ZD_TuoErSiTai,ZD_test2";
	$xianshi_ZD_num="4";
	$xianshi_KD_num="411";
	$shuoding_num="2";
	$maxrecord="50";
	$FormattingXianShi_idlist="1,6,2";
	$zu_all_list="";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="id,sys_id_login,sys_login,sys_id_zu,sys_id_fz,sys_yfzuid,sys_bh,sys_zt,sys_zt_bianhao,sys_nowbh,sys_huis,sys_id_bumen,sys_shenpi,sys_web_shenpi,sys_adddate,sys_adddate_g,sys_shenpi_all,sys_chaosong,sys_paixu,ZD_test,ZD_TuoErSiTai,ZD_test2";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,SQP_NaWaiBuYinSuXiangGuanXinXiYiLanBiao.id  from `SQP_NaWaiBuYinSuXiangGuanXinXiYiLanBiao`,(select @rownum:=0) as SQP_NaWaiBuYinSuXiangGuanXinXiYiLanBiao where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,sys_nowbh,ZD_test,ZD_TuoErSiTai,ZD_test2  from `SQP_NaWaiBuYinSuXiangGuanXinXiYiLanBiao` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      $sql .= " order by $px_ziduan $pxv";
      $rs = mysqli_query( $Conn, $sql );
      echo( "<div id='para'  tabindex='-1' class='tables'  style='border:0;border-bottom:1px solid #CCC;min-width:100%;width:411'>" );
	      ...
	      if ( $const_q_cak >= 0 ) {
			...
			echo( "<li class='cols1 shuodingli'  title='$now_id'  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;text-align:center;'></li>" );
              ...
              echo("<li  class='shuodingli  border_shuoding  ET_ZD_test2$now_id F_M_XS_2' ET='ET_ZD_test2'  xstypeid='2' name='ZD_test2' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['ZD_test2'])."</li>");
              echo("<li class='contentli ET_ZD_test$now_id F_M_XS_6' ET='ET_ZD_test'  xstypeid='6' name='ZD_test' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['ZD_test'])."</li>");
              echo("<li class='contentli ET_ZD_TuoErSiTai$now_id F_M_XS_6' ET='ET_ZD_TuoErSiTai'  xstypeid='6' name='ZD_TuoErSiTai' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['ZD_TuoErSiTai'])."</li>");
