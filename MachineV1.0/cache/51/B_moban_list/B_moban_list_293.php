...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="id,sys_re_id,sys_re_id_02,sys_nowid_guanxi_col,sys_databiaoYY,sys_GuanXiZDList,sys_zdlist,sys_yfzuid,sys_bh,sys_zt,sys_zt_bianhao,sys_nowbh,sys_huis,sys_id_bumen,sys_shenpi,sys_id_fz,sys_web_shenpi,sys_shenpi_all,sys_adddate,sys_adddate_g,sys_login,sys_id_login,sys_id_zu,sys_chaosong,sys_paixu,sys_gx_id_198,sys_gx_id_529";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,sys_guanxibiao.id  from `sys_guanxibiao`,(select @rownum:=0) as sys_guanxibiao where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,sys_nowbh  from `sys_guanxibiao` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
