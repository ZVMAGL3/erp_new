...
global $const_q_cak,$ToHtmlID,$const_q_shanc,$const_q_xiug,$nowkeyword,$px_ziduan,$pxv,$Conn,$hy;
	...
	$databiao="sys_zuall";
	$const_shaixuan="";
	$SYS_ALL_ziduan_list="id,sys_nowbh,lname1,tableid,sys_paixu,sys_id_login,sys_shenpi_all";
	$xianshi_ZD_num="5";
	$xianshi_KD_num="851";
	$shuoding_num="4";
	$maxrecord="20";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="id,sys_nowbh,lname1,tableid,sys_yfzuid,sys_huis,re_id,sys_id_bumen,sys_shenpi,sys_web_shenpi,sys_id_fz,sys_shenpi_all,sys_adddate,sys_adddate_g,sys_login,sys_id_login,sys_id_zu,sys_bh,sys_zt,sys_zt_bianhao,sys_chaosong,ZD_PaiXu,sys_paixu";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,sys_zuall.id  from `sys_zuall`,(select @rownum:=0) as sys_zuall where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,sys_nowbh,lname1,tableid,sys_paixu,sys_id_login,sys_shenpi_all  from `sys_zuall` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      $sql .= " order by $px_ziduan $pxv";
      $rs = mysqli_query( $Conn, $sql );
      echo( "<div id='para'  tabindex='-1' class='tables'  style='border:0;border-bottom:1px solid #CCC;min-width:100%;width: 851px'>" );
	      ...
	      if ( $const_q_cak >= 0 or $now_id_login == $bh ) {
			...
			//}
			echo( "  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;min-width:100%;width:851px'> " );
			echo( "<li class='cols1 shuodingli'  title='$now_id'  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;text-align:center;'></li>" );
              ...
              echo("<li  class='shuodingli   ET_lname1$now_id F_M_XS_1' ET='ET_lname1'  xstypeid='1' name='lname1' style='height:28px;line-height:28px;width:238px;'>".DeleteHtml($row['lname1'])."</li>");
              echo("<li  class='shuodingli  border_shuoding  ET_tableid$now_id F_M_XS_1' ET='ET_tableid'  xstypeid='1' name='tableid' style='height:28px;line-height:28px;width:103px;'>".DeleteHtml($row['tableid'])."</li>");
