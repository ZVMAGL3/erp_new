...
global $const_q_cak,$ToHtmlID,$const_q_shanc,$const_q_xiug,$nowkeyword,$px_ziduan,$pxv,$Connadmin,$hy;
	$r_cow_height="28";
	$databiao="msc_bumenlist";
	$const_shaixuan="";
	$SYS_ALL_ziduan_list="id,bumenname,parent";
	$xianshi_ZD_num="2";
	$xianshi_KD_num="243";
	$shuoding_num="2";
	$maxrecord="50";
	$FormattingXianShi_idlist="2";
	$zu_all_list="";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="id,bumenname,parent,up_id,down_id,quanxianlist,sys_id_zu,jib,sys_yfzuid,bianhao,sys_bh,sys_zt,sys_zt_bianhao,sys_nowbh,sys_huis,sys_id_bumen,sys_shenpi,sys_id_fz,sys_web_shenpi,sys_shenpi_all,sys_adddate,sys_adddate_g,sys_login,sys_id_login,sys_chaosong,sys_paixu";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,msc_bumenlist.id  from `msc_bumenlist`,(select @rownum:=0) as msc_bumenlist where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,bumenname,parent  from `msc_bumenlist` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      $sql .= " order by $px_ziduan $pxv";
      $rs = mysqli_query( $Connadmin, $sql );
      echo( "<div id='para'  tabindex='-1' class='tables'  style='border:0;border-bottom:1px solid #CCC;min-width:100%;width:243'>" );
	      ...
	      if ( $const_q_cak >= 0 ) {
			...
			echo( "<li class='cols1 shuodingli'  title='$now_id'  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;text-align:center;'></li>" );
              ...
              echo("<li id='c_tdtop1'  class='shuodingli   ET_bumenname$now_id F_M_XS_2' ET='ET_bumenname'  xstypeid='2' name='bumenname' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['bumenname'])."</li>");
              echo("<li  class='shuodingli  border_shuoding  ET_parent$now_id F_M_XS_2' ET='ET_parent'  xstypeid='2' name='parent' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['parent'])."</li>");
