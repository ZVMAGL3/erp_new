...
global $const_q_cak,$ToHtmlID,$const_q_shanc,$const_q_xiug,$nowkeyword,$px_ziduan,$pxv,$Conn,$hy;
	$r_cow_height="28";
	$databiao="SQP_MeiRiGongZuoHuiBaoBiao";
	$const_shaixuan="";
	$SYS_ALL_ziduan_list="id,ZD_XiangXiMiaoShu,sys_nowbh,ZD_HuiBaoRen,ZD_HuiBaoRiQi,ZD_WanChengQingKuang,ZD_JiFen,ZD_ShenYueRen,ZD_ShenYueShiJian,sys_id_login,sys_shenpi_all";
	$xianshi_ZD_num="8";
	$xianshi_KD_num="1360";
	$shuoding_num="2";
	$maxrecord="50";
	$FormattingXianShi_idlist="2,1,23,17";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="sys_id_zu,ZD_XiangXiMiaoShu,id,sys_id_login,sys_login,sys_id_fz,sys_yfzuid,sys_bh,sys_zt,sys_zt_bianhao,sys_nowbh,ZD_HuiBaoRen,ZD_HuiBaoRiQi,sys_huis,sys_id_bumen,sys_shenpi,sys_web_shenpi,sys_adddate_g,sys_adddate,sys_shenpi_all,ZD_WanChengQingKuang,ZD_JiFen,ZD_ShenYueRen,ZD_ShenYueShiJian,sys_chaosong,sys_paixu,sys_count_264";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,SQP_MeiRiGongZuoHuiBaoBiao.id  from `SQP_MeiRiGongZuoHuiBaoBiao`,(select @rownum:=0) as SQP_MeiRiGongZuoHuiBaoBiao where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,ZD_XiangXiMiaoShu,sys_nowbh,ZD_HuiBaoRen,ZD_HuiBaoRiQi,ZD_WanChengQingKuang,ZD_JiFen,ZD_ShenYueRen,ZD_ShenYueShiJian,sys_id_login,sys_shenpi_all  from `SQP_MeiRiGongZuoHuiBaoBiao` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      $sql .= " order by $px_ziduan $pxv";
      $rs = mysqli_query( $Conn, $sql );
      echo( "<div id='para'  tabindex='-1' class='tables'  style='border:0;border-bottom:1px solid #CCC;min-width:100%;width: 1360px'>" );
	      ...
	      if ( $const_q_cak >= 0 or $now_id_login == $bh ) {
			...
			//}
			echo( "  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;min-width:100%;width:1360px'> " );
			echo( "<li class='cols1 shuodingli'  title='$now_id'  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;text-align:center;'></li>" );
              ...
              echo("<li  class='shuodingli  border_shuoding  ET_ZD_HuiBaoRen$now_id F_M_XS_23' ET='ET_ZD_HuiBaoRen'  xstypeid='23' name='ZD_HuiBaoRen' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['ZD_HuiBaoRen'])."</li>");
              echo("<li  class='contentli ET_ZD_XiangXiMiaoShu$now_id F_M_XS_2' ET='ET_ZD_XiangXiMiaoShu'  xstypeid='2' name='ZD_XiangXiMiaoShu' style='height:28px;line-height:28px;width:600px;'>".DeleteHtml($row['ZD_XiangXiMiaoShu'])."</li>");
              echo("<li  class='contentli ET_ZD_HuiBaoRiQi$now_id F_M_XS_1' ET='ET_ZD_HuiBaoRiQi'  xstypeid='1' name='ZD_HuiBaoRiQi' style='height:28px;line-height:28px;width:70px;'>".DeleteHtml($row['ZD_HuiBaoRiQi'])."</li>");
              echo("<li  class='contentli ET_ZD_WanChengQingKuang$now_id F_M_XS_17' ET='ET_ZD_WanChengQingKuang'  xstypeid='17' name='ZD_WanChengQingKuang' style='height:28px;line-height:28px;width:60px;'>".DeleteHtml($row['ZD_WanChengQingKuang'])."</li>");
              ...
              echo("<li  class='contentli ET_ZD_ShenYueRen$now_id F_M_XS_1' ET='ET_ZD_ShenYueRen'  xstypeid='1' name='ZD_ShenYueRen' style='height:28px;line-height:28px;width:50px;'>".DeleteHtml($row['ZD_ShenYueRen'])."</li>");
              echo("<li  class='contentli ET_ZD_ShenYueShiJian$now_id F_M_XS_1' ET='ET_ZD_ShenYueShiJian'  xstypeid='1' name='ZD_ShenYueShiJian' style='height:28px;line-height:28px;width:70px;'>".DeleteHtml($row['ZD_ShenYueShiJian'])."</li>");
