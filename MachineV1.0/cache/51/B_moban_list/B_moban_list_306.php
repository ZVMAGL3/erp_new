...
global $const_q_cak,$ToHtmlID,$const_q_shanc,$const_q_xiug,$nowkeyword,$px_ziduan,$pxv,$Conn,$hy;
	...
	$databiao="SQP_CPK";
	$const_shaixuan="";
	$SYS_ALL_ziduan_list="id,sys_id_zu,sys_nowbh,sys_login,sys_chaosong,sys_paixu";
	$xianshi_ZD_num="5";
	$xianshi_KD_num="640";
	$shuoding_num="1";
	$maxrecord="50";
	$FormattingXianShi_idlist="15,1";
	$zu_all_list="";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="id,username,tel,card,startdate,enddate,beizhu,qq,sys_id_zu,pxdx,px_ks,px_js,sys_yfzuid,bianhao,sys_bh,sys_zt,sys_zt_bianhao,sys_nowbh,sys_huis,sys_id_bumen,sys_shenpi,sys_id_fz,sys_web_shenpi,sys_shenpi_all,sys_adddate,sys_adddate_g,sys_login,sys_id_login,sys_chaosong,sys_paixu,ZD_hhhh";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,SQP_CPK.id  from `SQP_CPK`,(select @rownum:=0) as SQP_CPK where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,sys_id_zu,sys_nowbh,sys_login,sys_chaosong,sys_paixu  from `SQP_CPK` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      $sql .= " order by $px_ziduan $pxv";
      $rs = mysqli_query( $Conn, $sql );
      echo( "<div id='para'  tabindex='-1' class='tables'  style='border:0;border-bottom:1px solid #CCC;min-width:100%;width: 640px'>" );
	      ...
	      if ( $const_q_cak >= 0 ) {
			...
			//}
			echo( "  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;min-width:100%;width:640px'> " );
			echo( "<li class='cols1 shuodingli'  title='$now_id'  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;text-align:center;'></li>" );
              ...
              echo("<li class='contentli ET_sys_paixu$now_id F_M_XS_1' ET='ET_sys_paixu'  xstypeid='1' name='sys_paixu' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['sys_paixu'])."</li>");
