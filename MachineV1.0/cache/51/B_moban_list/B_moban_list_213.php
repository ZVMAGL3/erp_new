...
global $const_q_cak,$ToHtmlID,$const_q_shanc,$const_q_xiug,$nowkeyword,$px_ziduan,$pxv,$Conn,$hy;
	$r_cow_height="28";
	$databiao="SQP_JiChuSheShiGuanLiTaiZhang";
	$const_shaixuan="";
	$SYS_ALL_ziduan_list="id,sys_nowbh,SheBeiMingChen,XingHaoGuiGe,ZhiZaoChangShang,ChuChangBianHao,SuoShuBuMen,ShiYongRiQi,BeiZhu,sys_id_login,sys_shenpi_all";
	$xianshi_ZD_num="8";
	$xianshi_KD_num="1150";
	$shuoding_num="2";
	$maxrecord="50";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="id,sys_id_login,sys_login,sys_id_zu,sys_id_fz,sys_yfzuid,sys_bh,sys_zt,sys_zt_bianhao,sys_nowbh,sys_huis,sys_id_bumen,sys_shenpi,sys_web_shenpi,sys_adddate,sys_adddate_g,sys_shenpi_all,SheBeiMingChen,XingHaoGuiGe,ZhiZaoChangShang,ChuChangBianHao,SuoShuBuMen,ShiYongRiQi,BeiZhu,sys_chaosong,sys_paixu";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,SQP_JiChuSheShiGuanLiTaiZhang.id  from `SQP_JiChuSheShiGuanLiTaiZhang`,(select @rownum:=0) as SQP_JiChuSheShiGuanLiTaiZhang where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,sys_nowbh,SheBeiMingChen,XingHaoGuiGe,ZhiZaoChangShang,ChuChangBianHao,SuoShuBuMen,ShiYongRiQi,BeiZhu,sys_id_login,sys_shenpi_all  from `SQP_JiChuSheShiGuanLiTaiZhang` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      $sql .= " order by $px_ziduan $pxv";
      $rs = mysqli_query( $Conn, $sql );
      echo( "<div id='para'  tabindex='-1' class='tables'  style='border:0;border-bottom:1px solid #CCC;min-width:100%;width: 1150px'>" );
	      ...
	      if ( $const_q_cak >= 0 or $now_id_login == $bh ) {
			...
			//}
			echo( "  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;min-width:100%;width:1150px'> " );
			echo( "<li class='cols1 shuodingli'  title='$now_id'  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;text-align:center;'></li>" );
              ...
              echo("<li  class='contentli ET_SuoShuBuMen$now_id F_M_XS_1' ET='ET_SuoShuBuMen'  xstypeid='1' name='SuoShuBuMen' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['SuoShuBuMen'])."</li>");
              ...
              echo("<li  class='contentli ET_BeiZhu$now_id F_M_XS_1' ET='ET_BeiZhu'  xstypeid='1' name='BeiZhu' style='height:28px;line-height:28px;width:100px;'>".DeleteHtml($row['BeiZhu'])."</li>");
