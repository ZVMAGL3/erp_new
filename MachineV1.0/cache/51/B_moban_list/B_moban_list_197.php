...
global $const_q_cak,$ToHtmlID,$const_q_shanc,$const_q_xiug,$nowkeyword,$px_ziduan,$pxv,$Conn,$hy;
	$r_cow_height="28";
	$databiao="SQP_YuanGongManYiDuDiaoChaBiao";
	$const_shaixuan="";
	$SYS_ALL_ziduan_list="id,username,tel,card,startdate,enddate,beizhu,qq,bianhao,sys_nowbh,sys_id_login,sys_shenpi_all";
	$xianshi_ZD_num="10";
	$xianshi_KD_num="1100";
	$shuoding_num="1";
	$maxrecord="50";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="id,username,tel,card,startdate,enddate,beizhu,qq,sys_id_zu,sys_yfzuid,bianhao,sys_bh,sys_zt,sys_zt_bianhao,sys_nowbh,sys_huis,sys_id_bumen,sys_shenpi,sys_id_fz,sys_web_shenpi,sys_shenpi_all,sys_adddate,sys_adddate_g,sys_login,sys_id_login,sys_chaosong,sys_paixu";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,SQP_YuanGongManYiDuDiaoChaBiao.id  from `SQP_YuanGongManYiDuDiaoChaBiao`,(select @rownum:=0) as SQP_YuanGongManYiDuDiaoChaBiao where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,username,tel,card,startdate,enddate,beizhu,qq,bianhao,sys_nowbh,sys_id_login,sys_shenpi_all  from `SQP_YuanGongManYiDuDiaoChaBiao` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      $sql .= " order by $px_ziduan $pxv";
      $rs = mysqli_query( $Conn, $sql );
      echo( "<div id='para'  tabindex='-1' class='tables'  style='border:0;border-bottom:1px solid #CCC;min-width:100%;width: 1100px'>" );
	      ...
	      if ( $const_q_cak >= 0 or $now_id_login == $bh ) {
			...
			//}
			echo( "  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;min-width:100%;width:1100px'> " );
			echo( "<li class='cols1 shuodingli'  title='$now_id'  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;text-align:center;'></li>" );
              ...
              echo("<li id='c_tdtop1'  class='contentli ET_username$now_id F_M_XS_1' ET='ET_username'  xstypeid='1' name='username' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['username'])."</li>");
              echo("<li id='c_tdtop1'  class='contentli ET_tel$now_id F_M_XS_1' ET='ET_tel'  xstypeid='1' name='tel' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['tel'])."</li>");
              echo("<li id='c_tdtop1'  class='contentli ET_card$now_id F_M_XS_1' ET='ET_card'  xstypeid='1' name='card' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['card'])."</li>");
              echo("<li id='c_tdtop1'  class='contentli ET_startdate$now_id F_M_XS_1' ET='ET_startdate'  xstypeid='1' name='startdate' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['startdate'])."</li>");
              echo("<li id='c_tdtop1'  class='contentli ET_enddate$now_id F_M_XS_1' ET='ET_enddate'  xstypeid='1' name='enddate' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['enddate'])."</li>");
              echo("<li id='c_tdtop1'  class='contentli ET_beizhu$now_id F_M_XS_1' ET='ET_beizhu'  xstypeid='1' name='beizhu' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['beizhu'])."</li>");
              echo("<li id='c_tdtop1'  class='contentli ET_qq$now_id F_M_XS_1' ET='ET_qq'  xstypeid='1' name='qq' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['qq'])."</li>");
              echo("<li id='c_tdtop1'  class='contentli ET_bianhao$now_id F_M_XS_1' ET='ET_bianhao'  xstypeid='1' name='bianhao' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['bianhao'])."</li>");
