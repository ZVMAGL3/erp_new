...
global $const_q_cak,$ToHtmlID,$const_q_shanc,$const_q_xiug,$nowkeyword,$px_ziduan,$pxv,$Conn,$hy;
	$r_cow_height="28";
	$databiao="SQP_JianCeHeCeLiangZiYuanTaiZhang";
	$const_shaixuan="";
	$SYS_ALL_ziduan_list="id,sys_nowbh,MingChen,GuiGe,ZhiZaoChangShang,ChuChangBianHao,ShiYongRiQi,ShiYongBuMen,GuanLiZeRenRen,BaoFeiRiQi";
	$xianshi_ZD_num="9";
	$xianshi_KD_num="1021";
	$shuoding_num="2";
	$maxrecord="50";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="sys_nowbh,MingChen,id,GuiGe,ZhiZaoChangShang,ChuChangBianHao,ShiYongRiQi,ShiYongBuMen,GuanLiZeRenRen,sys_id_zu,BaoFeiRiQi,px_js,sys_yfzuid,bianhao,sys_bh,sys_zt,sys_zt_bianhao,sys_huis,sys_id_bumen,sys_shenpi,sys_id_fz,sys_web_shenpi,sys_shenpi_all,sys_adddate,sys_adddate_g,sys_login,sys_id_login,sys_chaosong,sys_paixu";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,SQP_JianCeHeCeLiangZiYuanTaiZhang.id  from `SQP_JianCeHeCeLiangZiYuanTaiZhang`,(select @rownum:=0) as SQP_JianCeHeCeLiangZiYuanTaiZhang where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,sys_nowbh,MingChen,GuiGe,ZhiZaoChangShang,ChuChangBianHao,ShiYongRiQi,ShiYongBuMen,GuanLiZeRenRen,BaoFeiRiQi  from `SQP_JianCeHeCeLiangZiYuanTaiZhang` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      ...
      echo( "<div id='para'  tabindex='-1' class='tables'  style='border:0;border-bottom:1px solid #CCC;min-width:99%;'>" );
	      ...
	      if ( $const_q_cak >= 0 ) {
			...
			echo( "<li class='cols1 shuodingli'  title='$now_id'  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;text-align:center;'></li>" );
              ...
              echo("<li  class='shuodingli  border_shuoding  ET_MingChen$now_id F_M_XS_1' ET='ET_MingChen'  xstypeid='1' name='MingChen' style='height:28px;line-height:28px;width:150px;'>".DeleteHtml($row['MingChen'])."</li>");
              echo("<li class='contentli ET_GuiGe$now_id F_M_XS_1' ET='ET_GuiGe'  xstypeid='1' name='GuiGe' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['GuiGe'])."</li>");
              echo("<li class='contentli ET_ZhiZaoChangShang$now_id F_M_XS_1' ET='ET_ZhiZaoChangShang'  xstypeid='1' name='ZhiZaoChangShang' style='height:28px;line-height:28px;width:200px;'>".DeleteHtml($row['ZhiZaoChangShang'])."</li>");
              echo("<li class='contentli ET_ChuChangBianHao$now_id F_M_XS_1' ET='ET_ChuChangBianHao'  xstypeid='1' name='ChuChangBianHao' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['ChuChangBianHao'])."</li>");
              echo("<li class='contentli ET_ShiYongRiQi$now_id F_M_XS_1' ET='ET_ShiYongRiQi'  xstypeid='1' name='ShiYongRiQi' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['ShiYongRiQi'])."</li>");
              echo("<li class='contentli ET_ShiYongBuMen$now_id F_M_XS_1' ET='ET_ShiYongBuMen'  xstypeid='1' name='ShiYongBuMen' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['ShiYongBuMen'])."</li>");
              echo("<li class='contentli ET_GuanLiZeRenRen$now_id F_M_XS_1' ET='ET_GuanLiZeRenRen'  xstypeid='1' name='GuanLiZeRenRen' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['GuanLiZeRenRen'])."</li>");
              echo("<li class='contentli ET_BaoFeiRiQi$now_id F_M_XS_1' ET='ET_BaoFeiRiQi'  xstypeid='1' name='BaoFeiRiQi' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['BaoFeiRiQi'])."</li>");
