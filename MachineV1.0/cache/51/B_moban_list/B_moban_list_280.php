...
global $const_q_cak,$ToHtmlID,$const_q_shanc,$const_q_xiug,$nowkeyword,$px_ziduan,$pxv,$Conn,$hy;
	$r_cow_height="28";
	$databiao="SQP_GongZuoJiHua";
	$const_shaixuan="";
	$SYS_ALL_ziduan_list="id,sys_nowbh,GongZuoXiangMu,NaRongYaoQiu,JiaoQi,WanCheng";
	$xianshi_ZD_num="5";
	$xianshi_KD_num="1086";
	$shuoding_num="2";
	$maxrecord="50";
	$FormattingXianShi_idlist="1,2,8,17";
	$zu_all_list="";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="sys_nowbh,id,GongZuoXiangMu,NaRongYaoQiu,JiaoQi,sys_yfzuid,WanCheng,sys_id_zu,sys_bh,sys_zt,sys_zt_bianhao,sys_huis,sys_id_bumen,sys_shenpi,sys_id_fz,sys_web_shenpi,sys_shenpi_all,sys_adddate,sys_adddate_g,sys_login,sys_id_login,sys_gx_id_321,sys_chaosong,sys_paixu,sys_gx_id_198";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,SQP_GongZuoJiHua.id  from `SQP_GongZuoJiHua`,(select @rownum:=0) as SQP_GongZuoJiHua where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,sys_nowbh,GongZuoXiangMu,NaRongYaoQiu,JiaoQi,WanCheng  from `SQP_GongZuoJiHua` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      $sql .= " order by $px_ziduan $pxv";
      $rs = mysqli_query( $Conn, $sql );
      echo( "<div id='para'  tabindex='-1' class='tables'  style='border:0;border-bottom:1px solid #CCC;min-width:100%;width: 1086px'>" );
	      ...
	      if ( $const_q_cak >= 0 ) {
			...
			//}
			echo( "  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;min-width:100%;width:1086px'> " );
			echo( "<li class='cols1 shuodingli'  title='$now_id'  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;text-align:center;'></li>" );
              ...
              echo("<li  class='shuodingli  border_shuoding  ET_GongZuoXiangMu$now_id F_M_XS_1' ET='ET_GongZuoXiangMu'  xstypeid='1' name='GongZuoXiangMu' style='height:28px;line-height:28px;width:244px;'>".DeleteHtml($row['GongZuoXiangMu'])."</li>");
              echo("<li class='contentli ET_NaRongYaoQiu$now_id F_M_XS_2' ET='ET_NaRongYaoQiu'  xstypeid='2' name='NaRongYaoQiu' style='height:28px;line-height:28px;width:250px;'>".DeleteHtml($row['NaRongYaoQiu'])."</li>");
              echo("<li class='contentli ET_JiaoQi$now_id F_M_XS_8' ET='ET_JiaoQi'  xstypeid='8' name='JiaoQi' style='height:28px;line-height:28px;width:81px;'>".DeleteHtml($row['JiaoQi'])."</li>");
              echo("<li class='contentli ET_WanCheng$now_id F_M_XS_17' ET='ET_WanCheng'  xstypeid='17' name='WanCheng' style='height:28px;line-height:28px;width:131px;'>".DeleteHtml($row['WanCheng'])."</li>");
