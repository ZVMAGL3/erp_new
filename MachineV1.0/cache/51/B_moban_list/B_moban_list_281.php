...
global $const_q_cak,$ToHtmlID,$const_q_shanc,$const_q_xiug,$nowkeyword,$px_ziduan,$pxv,$Conn,$hy;
	$r_cow_height="28";
	$databiao="SQP_ChangYongWangZhi";
	$const_shaixuan="";
	$SYS_ALL_ziduan_list="id,sys_nowbh,ZD_WangZhanMingChen,ZD_WangZhi,ZD_WangZhanZhangHaoMiMa";
	$xianshi_ZD_num="4";
	$xianshi_KD_num="1618";
	$shuoding_num="2";
	$maxrecord="50";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="id,sys_id_login,sys_id_zu,sys_yfzuid,sys_bh,sys_zt,sys_zt_bianhao,sys_nowbh,sys_huis,sys_id_bumen,sys_shenpi,sys_id_fz,sys_web_shenpi,sys_shenpi_all,sys_adddate_g,ZD_WangZhanMingChen,ZD_WangZhi,ZD_WangZhanZhangHaoMiMa,sys_login,sys_adddate,sys_gx_id_321,sys_chaosong,sys_paixu,sys_gx_id_198";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,SQP_ChangYongWangZhi.id  from `SQP_ChangYongWangZhi`,(select @rownum:=0) as SQP_ChangYongWangZhi where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,sys_nowbh,ZD_WangZhanMingChen,ZD_WangZhi,ZD_WangZhanZhangHaoMiMa  from `SQP_ChangYongWangZhi` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      $sql .= " order by $px_ziduan $pxv";
      $rs = mysqli_query( $Conn, $sql );
      echo( "<div id='para'  tabindex='-1' class='tables'  style='border:0;border-bottom:1px solid #CCC;min-width:100%;width: 1618px'>" );
	      ...
	      if ( $const_q_cak >= 0 ) {
			...
			//}
			echo( "  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;min-width:100%;width:1618px'> " );
			echo( "<li class='cols1 shuodingli'  title='$now_id'  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;text-align:center;'></li>" );
              ...
              echo("<li  class='shuodingli  border_shuoding  ET_ZD_WangZhanMingChen$now_id F_M_XS_1' ET='ET_ZD_WangZhanMingChen'  xstypeid='1' name='ZD_WangZhanMingChen' style='height:28px;line-height:28px;width:238px;'>".DeleteHtml($row['ZD_WangZhanMingChen'])."</li>");
              echo("<li class='contentli ET_ZD_WangZhi$now_id F_M_XS_2' ET='ET_ZD_WangZhi'  xstypeid='2' name='ZD_WangZhi' style='height:28px;line-height:28px;width:470px;'>".DeleteHtml($row['ZD_WangZhi'])."</li>");
              echo("<li class='contentli ET_ZD_WangZhanZhangHaoMiMa$now_id F_M_XS_1' ET='ET_ZD_WangZhanZhangHaoMiMa'  xstypeid='1' name='ZD_WangZhanZhangHaoMiMa' style='height:28px;line-height:28px;width:530px;'>".DeleteHtml($row['ZD_WangZhanZhangHaoMiMa'])."</li>");
