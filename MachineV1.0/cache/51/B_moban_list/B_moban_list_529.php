...
global $const_q_cak,$ToHtmlID,$const_q_shanc,$const_q_xiug,$nowkeyword,$px_ziduan,$pxv,$Connadmin,$hy;
	$r_cow_height="28";
	$databiao="msc_user_hy";
	$const_shaixuan="";
	$SYS_ALL_ziduan_list="id,user_id,sys_yfzuid,state,SYS_GongHao,zhiwei_id,add_time,new_time,Remark";
	$xianshi_ZD_num="9";
	$xianshi_KD_num="971";
	$shuoding_num="1";
	$maxrecord="50";
	$FormattingXianShi_idlist="1,9";
	$zu_all_list="";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="id,user_id,sys_yfzuid,sys_huis,state,SYS_GongHao,zhiwei_id,add_time,new_time,Remark";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,msc_user_hy.id  from `msc_user_hy`,(select @rownum:=0) as msc_user_hy where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
      ...
      echo( "<div id='para'  tabindex='-1' class='tables'  style='border:0;border-bottom:1px solid #CCC;min-width:100%;width:971'>" );
	      ...
	      if ( $const_q_cak >= 0 ) {
			...
			echo( "<li class='cols1 shuodingli'  title='$now_id'  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;text-align:center;'></li>" );
              ...
              echo("<li class='contentli ET_user_id$now_id F_M_XS_1' ET='ET_user_id'  xstypeid='1' name='user_id' style='height:28px;line-height:28px;width:70px;'>".DeleteHtml($row['user_id'])."</li>");
              echo("<li class='contentli ET_sys_yfzuid$now_id F_M_XS_1' ET='ET_sys_yfzuid'  xstypeid='1' name='sys_yfzuid' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['sys_yfzuid'])."</li>");
              echo("<li class='contentli ET_state$now_id F_M_XS_1' ET='ET_state'  xstypeid='1' name='state' style='height:28px;line-height:28px;width:60px;'>".DeleteHtml($row['state'])."</li>");
              echo("<li class='contentli ET_SYS_GongHao$now_id F_M_XS_1' ET='ET_SYS_GongHao'  xstypeid='1' name='SYS_GongHao' style='height:28px;line-height:28px;width:110px;'>".DeleteHtml($row['SYS_GongHao'])."</li>");
              echo("<li class='contentli ET_zhiwei_id$now_id F_M_XS_1' ET='ET_zhiwei_id'  xstypeid='1' name='zhiwei_id' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['zhiwei_id'])."</li>");
              echo("<li class='contentli ET_add_time$now_id F_M_XS_9' ET='ET_add_time'  xstypeid='9' name='add_time' style='height:28px;line-height:28px;width:160px;'>".DeleteHtml($row['add_time'])."</li>");
              echo("<li class='contentli ET_new_time$now_id F_M_XS_9' ET='ET_new_time'  xstypeid='9' name='new_time' style='height:28px;line-height:28px;width:160px;'>".DeleteHtml($row['new_time'])."</li>");
              echo("<li class='endli ET_Remark$now_id F_M_XS_1' ET='ET_Remark'  xstypeid='1' name='Remark' style='height:28px;line-height:28px;width:60px;'>".DeleteHtml($row['Remark'])."</li>");
