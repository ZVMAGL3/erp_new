...
global $const_q_cak,$ToHtmlID,$const_q_shanc,$const_q_xiug,$nowkeyword,$px_ziduan,$pxv,$Conn,$hy;
	$r_cow_height="25";
	$databiao="SQP_ZhengCeFaGui";
	$const_shaixuan="";
	$SYS_ALL_ziduan_list="id,sys_nowbh,ZD_LianJie,ZD_FuJian,sys_adddate,sys_login,ZD_WenJianMingChen";
	$xianshi_ZD_num="6";
	$xianshi_KD_num="1240";
	$shuoding_num="2";
	$maxrecord="50";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="id,sys_id_zu,sys_id_fz,sys_yfzuid,sys_bh,sys_zt,sys_zt_bianhao,sys_nowbh,sys_huis,sys_id_bumen,sys_web_shenpi,ZD_LianJie,ZD_FuJian,sys_adddate,sys_adddate_g,sys_id_login,sys_login,sys_shenpi,sys_shenpi_all,sys_chaosong,sys_paixu,ZD_WenJianMingChen,sys_gx_id_198";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,SQP_ZhengCeFaGui.id  from `SQP_ZhengCeFaGui`,(select @rownum:=0) as SQP_ZhengCeFaGui where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,sys_nowbh,ZD_LianJie,ZD_FuJian,sys_adddate,sys_login,ZD_WenJianMingChen  from `SQP_ZhengCeFaGui` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      $sql .= " order by $px_ziduan $pxv";
      $rs = mysqli_query( $Conn, $sql );
      echo( "<div id='para'  tabindex='-1' class='tables'  style='border:0;border-bottom:1px solid #CCC;min-width:100%;width: 1240px'>" );
	      ...
	      if ( $const_q_cak >= 0 ) {
			...
			//}
			echo( "  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;min-width:100%;width:1240px'> " );
			echo( "<li class='cols1 shuodingli'  title='$now_id'  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;text-align:center;'></li>" );
              ...
              echo("<li  class='shuodingli  border_shuoding  ET_ZD_WenJianMingChen$now_id F_M_XS_1' ET='ET_ZD_WenJianMingChen'  xstypeid='1' name='ZD_WenJianMingChen' style='height:25px;line-height:25px;width:200px;'>".DeleteHtml($row['ZD_WenJianMingChen'])."</li>");
              echo("<li class='contentli ET_ZD_LianJie$now_id F_M_XS_1' ET='ET_ZD_LianJie'  xstypeid='1' name='ZD_LianJie' style='height:25px;line-height:25px;width:300px;'>".DeleteHtml($row['ZD_LianJie'])."</li>");
              echo("<li class='contentli ET_ZD_FuJian$now_id F_M_XS_6' ET='ET_ZD_FuJian'  xstypeid='6' name='ZD_FuJian' style='height:25px;line-height:25px;width:200px;'>".DeleteHtml($row['ZD_FuJian'])."</li>");
              echo("<li class='contentli ET_sys_adddate$now_id F_M_XS_1' ET='ET_sys_adddate'  xstypeid='1' name='sys_adddate' style='height:25px;line-height:25px;width:80px;'>".DeleteHtml($row['sys_adddate'])."</li>");
              echo("<li class='contentli ET_sys_login$now_id F_M_XS_1' ET='ET_sys_login'  xstypeid='1' name='sys_login' style='height:25px;line-height:25px;width:80px;'>".DeleteHtml($row['sys_login'])."</li>");
