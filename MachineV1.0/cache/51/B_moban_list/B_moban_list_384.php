...
global $const_q_cak,$ToHtmlID,$const_q_shanc,$const_q_xiug,$nowkeyword,$px_ziduan,$pxv,$Connadmin,$hy;
	...
	$databiao="msc_huob";
	$const_shaixuan="";
	$SYS_ALL_ziduan_list="id,danwei,h_name";
	$xianshi_ZD_num="2";
	$xianshi_KD_num="243";
	$shuoding_num="1";
	$maxrecord="50";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="id,danwei,sys_yfzuid,sys_login,sys_bh,sys_zt,sys_zt_bianhao,sys_nowbh,sys_huis,paixu,h_name,sys_id_bumen,sys_shenpi,sys_id_fz,sys_web_shenpi,sys_shenpi_all,sys_adddate_g,sys_id_login,sys_id_zu,sys_adddate,sys_chaosong,sys_paixu";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,msc_huob.id  from `msc_huob`,(select @rownum:=0) as msc_huob where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,danwei,h_name  from `msc_huob` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      ...
      echo( "<div id='para'  tabindex='-1' class='tables'  style='border:0;border-bottom:1px solid #CCC;min-width:99%;'>" );
	      ...
	      if ( $const_q_cak >= 0 ) {
			...
			echo( "<li class='cols1 shuodingli'  title='$now_id'  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;text-align:center;'></li>" );
              ...
              echo("<li class='contentli ET_danwei$now_id F_M_XS_2' ET='ET_danwei'  xstypeid='2' name='danwei' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['danwei'])."</li>");
              echo("<li class='contentli ET_h_name$now_id F_M_XS_2' ET='ET_h_name'  xstypeid='2' name='h_name' style='height:28px;line-height:28px;width:80px;'>".DeleteHtml($row['h_name'])."</li>");
