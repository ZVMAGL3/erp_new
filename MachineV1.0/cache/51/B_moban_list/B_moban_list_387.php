...
global $const_q_cak,$ToHtmlID,$const_q_shanc,$const_q_xiug,$nowkeyword,$px_ziduan,$pxv,$Conn,$hy;
	$r_cow_height="28";
	$databiao="SQP_ShengChanGongYiLiuCheng";
	$const_shaixuan="";
...
if ( "1" . $databiao == "1" ) {
   ...
   $sql2 = "select (@rownum:=@rownum+1) as rownum,SQP_ShengChanGongYiLiuCheng.id  from `SQP_ShengChanGongYiLiuCheng`,(select @rownum:=0) as SQP_ShengChanGongYiLiuCheng where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,sys_nowbh  from `SQP_ShengChanGongYiLiuCheng` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
