...
global $const_q_cak,$ToHtmlID,$const_q_shanc,$const_q_xiug,$nowkeyword,$px_ziduan,$pxv,$Conn,$hy;
	$r_cow_height="28";
	$databiao="SQP_XiTongGaiJin";
	$const_shaixuan="";
	$SYS_ALL_ziduan_list="id,sys_nowbh,ZD_WenJianBianHao,ZD_GaiJinWenTiDianMiaoShu,ZD_XiuGaiWanCheng,sys_id_login,sys_shenpi_all";
	$xianshi_ZD_num="4";
	$xianshi_KD_num="1175";
	$shuoding_num="2";
	$maxrecord="50";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="sys_nowbh,ZD_WenJianBianHao,ZD_GaiJinWenTiDianMiaoShu,sys_login,ZD_XiuGaiWanCheng,id,sys_id_login,sys_id_zu,sys_id_fz,sys_yfzuid,sys_bh,sys_zt,sys_zt_bianhao,sys_huis,sys_id_bumen,sys_shenpi,sys_web_shenpi,sys_adddate,sys_adddate_g,sys_shenpi_all,sys_chaosong,sys_paixu";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,SQP_XiTongGaiJin.id  from `SQP_XiTongGaiJin`,(select @rownum:=0) as SQP_XiTongGaiJin where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,sys_nowbh,ZD_WenJianBianHao,ZD_GaiJinWenTiDianMiaoShu,ZD_XiuGaiWanCheng,sys_id_login,sys_shenpi_all  from `SQP_XiTongGaiJin` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      $sql .= " order by $px_ziduan $pxv";
      $rs = mysqli_query( $Conn, $sql );
      echo( "<div id='para'  tabindex='-1' class='tables'  style='border:0;border-bottom:1px solid #CCC;min-width:100%;width: 1175px'>" );
	      ...
	      if ( $const_q_cak >= 0 or $now_id_login == $bh ) {
			...
			//}
			echo( "  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;min-width:100%;width:1175px'> " );
			echo( "<li class='cols1 shuodingli'  title='$now_id'  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;text-align:center;'></li>" );
              ...
              echo("<li  class='shuodingli  border_shuoding  ET_ZD_WenJianBianHao$now_id F_M_XS_1' ET='ET_ZD_WenJianBianHao'  xstypeid='1' name='ZD_WenJianBianHao' style='height:28px;line-height:28px;width:357px;'>".DeleteHtml($row['ZD_WenJianBianHao'])."</li>");
              echo("<li  class='contentli ET_ZD_GaiJinWenTiDianMiaoShu$now_id F_M_XS_2' ET='ET_ZD_GaiJinWenTiDianMiaoShu'  xstypeid='2' name='ZD_GaiJinWenTiDianMiaoShu' style='height:28px;line-height:28px;width:356px;'>".DeleteHtml($row['ZD_GaiJinWenTiDianMiaoShu'])."</li>");
              echo("<li  class='contentli ET_ZD_XiuGaiWanCheng$now_id F_M_XS_19' ET='ET_ZD_XiuGaiWanCheng'  xstypeid='19' name='ZD_XiuGaiWanCheng' style='height:28px;line-height:28px;width:82px;'>".DeleteHtml($row['ZD_XiuGaiWanCheng'])."</li>");
