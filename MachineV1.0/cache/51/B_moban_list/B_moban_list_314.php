...
global $const_q_cak,$ToHtmlID,$const_q_shanc,$const_q_xiug,$nowkeyword,$px_ziduan,$pxv,$Conn,$hy;
	$r_cow_height="28";
	$databiao="SQP_ZhaoPinGongYingShang";
	$const_shaixuan="";
	$SYS_ALL_ziduan_list="id,sys_nowbh,QuDaoMingChen,WangZhi,DiZhi,ZhangHaoMiMa,LeiXing,ZhaoPinJiQiao";
	$xianshi_ZD_num="7";
	$xianshi_KD_num="1183";
	$shuoding_num="2";
	$maxrecord="50";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="id,sys_id_login,sys_login,sys_id_zu,sys_yfzuid,sys_bh,sys_zt,sys_zt_bianhao,sys_nowbh,sys_huis,sys_id_bumen,sys_shenpi,sys_id_fz,sys_web_shenpi,sys_adddate,sys_adddate_g,sys_shenpi_all,QuDaoMingChen,WangZhi,DiZhi,ZhangHaoMiMa,LeiXing,ZhaoPinJiQiao,sys_chaosong,sys_paixu";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,SQP_ZhaoPinGongYingShang.id  from `SQP_ZhaoPinGongYingShang`,(select @rownum:=0) as SQP_ZhaoPinGongYingShang where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,sys_nowbh,QuDaoMingChen,WangZhi,DiZhi,ZhangHaoMiMa,LeiXing,ZhaoPinJiQiao  from `SQP_ZhaoPinGongYingShang` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      $sql .= " order by $px_ziduan $pxv";
      $rs = mysqli_query( $Conn, $sql );
      echo( "<div id='para'  tabindex='-1' class='tables'  style='border:0;border-bottom:1px solid #CCC;min-width:100%;width: 1183px'>" );
	      ...
	      if ( $const_q_cak >= 0 ) {
			...
			//}
			echo( "  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;min-width:100%;width:1183px'> " );
			echo( "<li class='cols1 shuodingli'  title='$now_id'  style='height:" . $r_cow_height . "px;line-height:" . $r_cow_height . "px;text-align:center;'></li>" );
              ...
              echo("<li  class='shuodingli  border_shuoding  ET_QuDaoMingChen$now_id F_M_XS_1' ET='ET_QuDaoMingChen'  xstypeid='1' name='QuDaoMingChen' style='height:28px;line-height:28px;width:150px;'>".DeleteHtml($row['QuDaoMingChen'])."</li>");
              echo("<li class='contentli ET_WangZhi$now_id F_M_XS_1' ET='ET_WangZhi'  xstypeid='1' name='WangZhi' style='height:28px;line-height:28px;width:200px;'>".DeleteHtml($row['WangZhi'])."</li>");
              echo("<li class='contentli ET_DiZhi$now_id F_M_XS_1' ET='ET_DiZhi'  xstypeid='1' name='DiZhi' style='height:28px;line-height:28px;width:200px;'>".DeleteHtml($row['DiZhi'])."</li>");
              echo("<li class='contentli ET_ZhangHaoMiMa$now_id F_M_XS_1' ET='ET_ZhangHaoMiMa'  xstypeid='1' name='ZhangHaoMiMa' style='height:28px;line-height:28px;width:150px;'>".DeleteHtml($row['ZhangHaoMiMa'])."</li>");
              echo("<li class='contentli ET_LeiXing$now_id F_M_XS_1' ET='ET_LeiXing'  xstypeid='1' name='LeiXing' style='height:28px;line-height:28px;width:100px;'>".DeleteHtml($row['LeiXing'])."</li>");
              echo("<li class='contentli ET_ZhaoPinJiQiao$now_id F_M_XS_2' ET='ET_ZhaoPinJiQiao'  xstypeid='2' name='ZhaoPinJiQiao' style='height:28px;line-height:28px;width:200px;'>".DeleteHtml($row['ZhaoPinJiQiao'])."</li>");
