<?php
		...
		$Table_name="SQP_FenGongSiGuanLi";
		$sys_re_id_02="255";
		$getdate=getdate();
		...
		$sql = "select * From `SQP_FenGongSiGuanLi` where `id`='$strmk_id' ";
...
if ( strpos($const_q_tianj, "255") !== false  ) { //有添加权限时
       echo"<ul>
          ...
          <input id='SYS_submit' value='提交' type='button' title='Ctrl+Enter提交' class='button button_ADD' style='width:90%'  SYS_Company_id='51' strmk_id='' firstinputname='id' bitian_Arry='' databiao='SQP_FenGongSiGuanLi' Wuchongfu_Arry=''  onclick=data_add_arrys(this,'#post_form','$ToHtmlID')   onkeydown='EnterPress(event,this,this.click)' />
  
          <font id='editsuccess' class='font_red'></font></li>
        </ul>";
}
		echo"<input type='hidden' name='re_id' id='re_id' value='255' />
		
		</div>
		</form>
		<div id='clonecopy2'>&nbsp;</div><script>YanZhen_ChongFu_ZuLoad(0,'','SQP_FenGongSiGuanLi','$ToHtmlID');form_add_copy('$ToHtmlID');inputfocusfirst('#".$ToHtmlID."_content_foot .htmlleirong','id');form_weikong('#post_form','DeskMenuDiv255');</script>";
