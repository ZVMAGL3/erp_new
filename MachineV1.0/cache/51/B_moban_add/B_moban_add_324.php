<?php
		...
		$Table_name="SQP_JianCeHeCeLiangZiYuanNaXiaoJiLu";
		$sys_re_id_02="324";
		$getdate=getdate();
...
if ( strpos($const_q_tianj, "324") !== false  ) { //有添加权限时
       echo"<ul>
          ...
          <input type='hidden' id='sys_postzd_list' name='sys_postzd_list' value='sys_gx_id_204'/>
  
          <input id='SYS_submit' value='提交' type='button' title='Ctrl+Enter提交' class='button button_ADD' style='width:90%'  SYS_Company_id='51' strmk_id='' firstinputname='id' bitian_Arry='' databiao='SQP_JianCeHeCeLiangZiYuanNaXiaoJiLu' Wuchongfu_Arry=''  onclick=data_add_arrys(this,'#post_form','$ToHtmlID')   onkeydown='EnterPress(event,this,this.click)' />
  
          <font id='editsuccess' class='font_red'></font></li>
        </ul>";
}
		echo"<input type='hidden' name='re_id' id='re_id' value='324' />
		
		</div>
		</form>
		<div id='clonecopy2'>&nbsp;</div><script>YanZhen_ChongFu_ZuLoad(0,'','SQP_JianCeHeCeLiangZiYuanNaXiaoJiLu','$ToHtmlID');form_add_copy('$ToHtmlID');inputfocusfirst('#".$ToHtmlID."_content_foot .htmlleirong','id');form_weikong('#post_form','DeskMenuDiv324');</script>";
