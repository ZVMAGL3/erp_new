...
if ( $act == 'add_mobile' ) { //当接收到处理指令时
    //----------------------------------查询是否有重复值
    //echo $hy;
    if ( isset( $_POST[ 'sys_GuoChengMingChen' ] ) )$sys_GuoChengMingChen = $_POST[ 'sys_GuoChengMingChen' ]; //过程名称
    ...
    if ( $sys_GuoChengMingChen . '1' == '1' ) {
        ...
        $sql = $rs = $record_count = '';
        $sql = "select id From `$tablename` where ($colsname='$sys_GuoChengMingChen') and sys_yfzuid='$hy' order by id Asc";
        //echo $sql.'_'; 
        ...
        if ( $record_count > 0 ) { //有重复值时
            echo "已有\"" . $sys_GuoChengMingChen . "\",禁止添加。";
        } else { //允许添加
            //echo"可以添加";
            $sql = $rs = '';
            $nowdata = date( 'Y-m-d H:i:s' );

            $sys_postzd_list = "$colsname,sys_id_standard,sys_huis,sys_id_login,sys_login,sys_yfzuid,sys_id_fz,sys_id_bumen,sys_adddate"; //加上系统默认值
            $sys_postvalue_list = "'$sys_GuoChengMingChen','$sys_id_standard','0','$bh','$SYS_UserName','$hy','$const_id_fz','$const_id_bumen','$nowdata'";
    ...
    <form autocomplete='off' οnsubmit='return false' οnkeydοwn="if(event.keyCode==13){return false;}">
        <div id='mobanaddbox' class='NowULTable nocopytext' style='text-align:right;width:100%;margin-top: 95px;margin-bottom: 90px;'> 
            ...
            <ul>
                <li class='cols01'> &nbsp;</li>
                <li class='cols02'>
                    <input id='reset_add' type='reset' value='重填' tabindex=-1 style='width:50px;margin-left: 2px;' class='button button_reset' onclick=inputfocusfirst('#mobanaddbox','tiaok') />
                    <input id='SYS_submit' value='提交' type='button' title='Ctrl+Enter提交' class='button button_ADD' style='width:100px'   onclick="add_edit_mobile(this,'<?php echo $phpstart ?>_add.php','add_mobile','<?php echo $phpstart ?>.php?id=<?php echo $id ?>')"    />
