...
if ( $act == 'edit_mobile' ) { //当接收到处理指令时
    ...
    if ( $name . '1' == '1' ) {
        ...
        if ( $record_count > 0 ) { //有重复值时
            ...
            $sql = "UPDATE  `$tablename`  set `$colsname` ='$name',`number`='$number',`sys_beizhu`='$sys_beizhu',`sys_adddate_g`='$nowdate' WHERE id='$id' ";
            //--------------------------------------以下为执行修改
    ...
    $sql = $rs = $row = '';
    if ( $id > 0 ) {
        $sql = "select id,number,$colsname,sys_beizhu,sys_adddate,sys_adddate_g From `$tablename` where id='$id' and sys_yfzuid='$hy' order by id Asc";
        //echo $sql.'_'; 
        $rs = mysqli_query( $Connadmin, $sql );
        //$record_count = mysqli_num_rows( $rs ); //统计总记录数
    ...
    <form autocomplete='off' οnsubmit='return false' οnkeydοwn="if(event.keyCode==13){return false;}">
        <div id='mobanaddbox' class='NowULTable nocopytext' style='text-align:right;width:100%;margin-top:75px;margin-bottom: 90px;'>
            ...
            <ul>
                ...
                <li class='cols02'>
                    <input type='text' typeid='1' name='number' id='number' placeholder="统一社会信用代码" class='addboxinput inputfocus'  value='<?php echo $number ?>'  style='width:90%' />
           ...
           <!--   !-->
            ...
            <ul>
                <li class='cols01'> &nbsp;</li>
                <li class='cols02'>
                    <input id='SYS_submit' value='提 交' type='button' title='Ctrl+Enter提交' class='button button_ADD' style='width:100px'   onclick="add_edit_mobile(this,'<?php echo $phpstart ?>_edit.php','edit_mobile','<?php echo $phpstart ?>_terms.php?id=<?php echo $id ?>')"    />
                </li>
            </ul>
            <ul>
                <li class='cols01'> &nbsp;</li>
