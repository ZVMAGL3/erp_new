...
function foots(){//分页
  global $Conn,$nowkeyword,$const_q_cak,$re_id,$page_count,$page,$ToHtmlID,$record_count,$hy;//全局变量
 ...
 if ($const_q_cak>-1){//有权限时
  ...
  if ($databiao.'1'!='1' ){
    ...
    $sql2=sql_all_counts($databiao,$nowkeyword,0,0);
	  //echo $sql2;
   ...
   echo "<script>$('#page_c').val('$page_count');$('#page_v').attr({'cs':'$page','value':'$page','pgc':'$page_count'}); $('#page_count').html('".$page_count."');$('#record_count').html('Σ&nbsp;$record_count');Fpage($page,'ok','$ToHtmlID'); </script>";
};  
