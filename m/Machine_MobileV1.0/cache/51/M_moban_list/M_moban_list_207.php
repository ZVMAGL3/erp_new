...
$r_cow_height="28";
$databiao="SQP_ShengChanTongZhiDan";
$const_shaixuan="";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="id,sys_id_login,sys_login,sys_id_zu,sys_id_fz,sys_yfzuid,sys_bh,sys_zt,sys_zt_bianhao,sys_nowbh,sys_huis,sys_id_bumen,sys_shenpi,sys_web_shenpi,sys_adddate,sys_adddate_g,sys_shenpi_all,sys_chaosong,sys_paixu,ZD_test";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,SQP_ShengChanTongZhiDan.id  from `SQP_ShengChanTongZhiDan`,(select @rownum:=0) as SQP_ShengChanTongZhiDan where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,sys_id_login,sys_shenpi_all  from `SQP_ShengChanTongZhiDan` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
   ...
   echo '<script>$("#loadingtimes").html("'.$ymdate.'ms");list_data_end_mobile("'.$sys_const_pagetype.'","DeskMenuDiv207");</script>';
};
