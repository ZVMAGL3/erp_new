...
if ( "1" . $databiao == "1" ) {
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,ZD_XingMing,sys_id_login,sys_shenpi_all  from `SQP_JianLiZhongXin` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      ...
      while ( $row = mysqli_fetch_array( $rs ) ) {
        ...
        if ( $const_q_cak >= 0 or $now_id_login == $bh ) {
            ...
            $tdqaqclass = "tdsclass" . $i;
              ...
              echo("<font  style=width:60px class='F_M_XS_1 ET_ZD_XingMing$now_id'>".DeleteHtml($row['ZD_XingMing'])."&nbsp;</font>");
              echo("</a>");
              echo("</li>");
   ...
   echo '<script>$("#loadingtimes").html("'.$ymdate.'ms");list_data_end_mobile("'.$sys_const_pagetype.'","DeskMenuDiv315");</script>';
};
