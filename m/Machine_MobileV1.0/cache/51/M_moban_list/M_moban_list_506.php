...
$const_shaixuan="";
$SYS_ALL_ziduan_list="id,ZD_XingMing,ZD_DangYueShiFa,sys_id_login,sys_shenpi_all";
$xianshi_ZD_num="2";
$xianshi_KD_num="2438";
$shuoding_num_list="";
	   $maxrecord="15";
...
if ( "1" . $databiao == "1" ) {
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,ZD_XingMing,ZD_DangYueShiFa,sys_id_login,sys_shenpi_all  from `sys_GongZiBiao` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      ...
      while ( $row = mysqli_fetch_array( $rs ) ) {
        ...
        if ( $const_q_cak >= 0 or $now_id_login == $bh ) {
            ...
            $tdqaqclass = "tdsclass" . $i;
              ...
              echo("<font  style=width:70px class='F_M_XS_1 ET_ZD_DangYueShiFa$now_id'>".DeleteHtml($row['ZD_DangYueShiFa'])."&nbsp;</font>");
              echo("</a>");
              echo("</li>");
   ...
   echo '<script>$("#loadingtimes").html("'.$ymdate.'ms");list_data_end_mobile("'.$sys_const_pagetype.'","DeskMenuDiv506");</script>';
};
