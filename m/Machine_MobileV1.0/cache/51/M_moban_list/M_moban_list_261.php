...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="id,sys_GuoChengMingChen,xu,quanxian,sys_yfzuid,sys_huis,sys_id_bumen,sys_id_fz,sys_bh,sys_zt,sys_zt_bianhao,sys_nowbh,sys_web_shenpi,sys_shenpi,sys_shenpi_all,sys_adddate,sys_adddate_g,sys_login,sys_id_login,sys_id_zu,sys_chaosong,sys_paixu";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,sys_menubigclass.id  from `sys_menubigclass`,(select @rownum:=0) as sys_menubigclass where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,sys_id_login,sys_shenpi_all  from `sys_menubigclass` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
   ...
   echo '<script>$("#loadingtimes").html("'.$ymdate.'ms");list_data_end_mobile("'.$sys_const_pagetype.'","DeskMenuDiv261");</script>';
};
