...
$const_shaixuan="";
$SYS_ALL_ziduan_list="id,ZD_XingMing,ZD_NianFen,ZD_JiaBanGongZi,sys_id_login,sys_shenpi_all";
$xianshi_ZD_num="2";
$xianshi_KD_num="1658";
$shuoding_num_list="ZD_JiaBanGongZi";
	   $maxrecord="15";
...
if ( "1" . $databiao == "1" ) {
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,ZD_XingMing,ZD_NianFen,ZD_JiaBanGongZi,sys_id_login,sys_shenpi_all,ZD_JiaBanGongZi  from `SQP_JiaBanDiaoXiuShiJianHuiZong` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
      ...
      while ( $row = mysqli_fetch_array( $rs ) ) {
        ...
        if ( $const_q_cak >= 0 or $now_id_login == $bh ) {
            ...
            $tdqaqclass = "tdsclass" . $i;
              ...
              echo("<font  style=width:50px class='F_M_XS_1 ET_ZD_NianFen$now_id'>".DeleteHtml($row['ZD_NianFen'])."&nbsp;</font>");
              echo("</a>");
   ...
   echo '<script>$("#loadingtimes").html("'.$ymdate.'ms");list_data_end_mobile("'.$sys_const_pagetype.'","DeskMenuDiv507");</script>';
};
