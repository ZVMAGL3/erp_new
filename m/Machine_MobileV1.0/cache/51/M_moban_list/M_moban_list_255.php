...
$shuoding_num_list="";
	   ...
	   $zu_all_list="348=上海市,356=云南省,344=内蒙古自治区,316=北京市,360=台湾,342=吉林省,351=四川省,352=天津市,353=天津市,345=宁夏,317=安徽省,347=山东省,349=山西省,322=广东省,323=广西省,354=新疆维吾尔自治区,332=江苏省,341=江西省,326=河北省,329=河南省,357=浙江省,325=海南省,330=湖北省,331=湖南省,359=澳门,321=甘肃省,320=福建省,355=西藏自治区,324=贵州省,343=辽宁省,319=重庆市,350=陕西省,346=青海省,358=香港,327=黑龙江省,";
...
if ( "1" . $databiao == "1" ) {
   ...
   $Tablecol_list="id,username,tel,fax,address,fzr,card,zphname,zph_web,email,startdate,enddate,beizhu,qq,sys_id_zu,sys_yfzuid,bianhao,sys_bh,sys_zt,sys_zt_bianhao,sys_nowbh,sys_huis,sys_id_bumen,sys_shenpi,website,photo,sys_web_shenpi,sys_id_fz,sys_shenpi_all,sys_adddate,sys_adddate_g,sys_login,sys_id_login,sys_chaosong,sys_paixu";

   $sql2 = "select (@rownum:=@rownum+1) as rownum,SQP_FenGongSiGuanLi.id  from `SQP_FenGongSiGuanLi`,(select @rownum:=0) as SQP_FenGongSiGuanLi where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,sys_id_login,sys_shenpi_all  from `SQP_FenGongSiGuanLi` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
   ...
   echo '<script>$("#loadingtimes").html("'.$ymdate.'ms");list_data_end_mobile("'.$sys_const_pagetype.'","DeskMenuDiv255");</script>';
};
