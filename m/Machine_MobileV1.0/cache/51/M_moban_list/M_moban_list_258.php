...
if ( "1" . $databiao == "1" ) {
   ...
   $sql2 = "select (@rownum:=@rownum+1) as rownum,SQP_ZhiZeQuanXian.id  from `SQP_ZhiZeQuanXian`,(select @rownum:=0) as SQP_ZhiZeQuanXian where  sys_yfzuid='$hy' and sys_huis='$huis' "; //这里得到查询id清单的sql
   ...
   if ( !$sql_all_id_list ) {
      ...
      $sql = "select  id,sys_id_login,sys_shenpi_all  from `SQP_ZhiZeQuanXian` where id in($sql_all_id_list) and sys_yfzuid='$hy' and sys_huis='$huis'";
   ...
   echo '<script>$("#loadingtimes").html("'.$ymdate.'ms");list_data_end_mobile("'.$sys_const_pagetype.'","DeskMenuDiv258");</script>';
};
