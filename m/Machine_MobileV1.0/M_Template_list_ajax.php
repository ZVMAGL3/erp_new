...
function listdata() {
    ...
    if ( $nowkeyword . '1' != '1' ) { //不为空时
        $sql = "select id,sys_keyword_en,$colsname,sys_adddate From `$tablename` where sys_huis='$huis' and $colsname like '%$nowkeyword%' and sys_Id_MenuBigClass='$parent_id' order by id Asc";
    } else {
        $sql = "select id,sys_keyword_en,$colsname,sys_adddate From `$tablename` where sys_huis='$huis' and sys_Id_MenuBigClass='$parent_id'  order by id Asc";
    }
    ...
    if ( $record_count == 0 ) {
        ...
        while ( $row = mysqli_fetch_array( $rs ) ) {
            ...
            $sys_keyword_en = $row[ 'sys_keyword_en' ]; //标准编号
            ...
            if ( $huis == 1 ) {
                ...
                $nowhuishtml = "<dd class='hui'>$sys_keyword_en</dd>";//$adddate
            }
            $listdata .= '<li><a href="' . $edithref . '"><label><input type="checkbox" class="idd" name="ID" value="' . $id . '" onchange="checkedGroup_mobile(this)"><i class="fa fa-11-4"></i>&nbsp;&nbsp;</label><strong>' . $name . '</strong>' . $nowhuishtml . '</a></li>';
        }
    }
